...
def get_conversation_state() -> Dict:
    """Get conversation state from session with automatic cleanup

    Returns:
        Conversation state dictionary with summaries and recent messages
    """
...
def compress_messages(messages: List[Dict], threshold: int) -> str:
    """Compress a batch of messages into a summary

    Args:
        messages: Messages to compress
        threshold: Number of messages to compress

    Returns:
        Summary of the messages
    """
    try:
        # Adaptive max_tokens based on number of messages
        num_messages = len(messages)
        if num_messages <= 4:
            max_tokens = COMPRESSION_SHORT_MAX_TOKENS
        elif num_messages <= 10:
            max_tokens = COMPRESSION_MEDIUM_MAX_TOKENS
        else:
            max_tokens = COMPRESSION_LONG_MAX_TOKENS
...
Output: Single sentence with all critical facts."""
        ...
        response = client.chat.completions.create(
            ...
            messages=messages_to_send,
            temperature=0.3,
            max_tokens=COMPRESSION_RECURSIVE_MAX_TOKENS
        )
...
def generate_dynamic_prompt(response_format: str, intelligent_mode: bool = False) -> str:
    """Generate system prompt based on format and mode

    Args:
        response_format: Response format (plain, json, markdown, xml)
        intelligent_mode: Enable intelligent mode
...
def get_ai_response(user_message: str, response_format: str = "plain",
                   temperature: float = OPENAI_TEMPERATURE, intelligent_mode: bool = False,
                   max_tokens: int = OPENAI_MAX_TOKENS, compression_enabled: bool = True,
                   threshold: int = DEFAULT_COMPRESSION_THRESHOLD,
                   keep_recent: int = DEFAULT_RECENT_KEEP) -> Dict:
    ...
    Args:
        ...
        compression_enabled: Enable conversation compression
        threshold: Number of messages before compression
        keep_recent: Number of recent messages to keep uncompressed

    Returns:
        Response dictionary with compression stats
    """
    try:
        # Get system prompt
        system_prompt = generate_dynamic_prompt(response_format, intelligent_mode)
...
def chat():
    ...
    try:
        ...
        result = get_ai_response(
            user_message, response_format, temperature, intelligent_mode, max_tokens,
