...
RESPONSE_PROMPTS = {
    "plain": """You are an intelligent AI assistant with a special ability: you know when you need more information.
...
def get_ai_response(user_message, response_format="plain", fields=None):
    """
    ...
    try:
        ...
        # Clean up response for JSON and XML formats (remove markdown code blocks if present)
        if response_format == "json":
            ai_response = clean_json_response(ai_response)
        elif response_format == "xml":
            ai_response = clean_xml_response(ai_response)
...
def chat():
    ...
    try:
        ...
        # Get AI response with specified format and fields
        ai_response = get_ai_response(user_message, response_format, fields)
