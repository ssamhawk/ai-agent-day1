...
chatForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const reviewMode = reviewModeCheckbox.checked;
...
function displayTemperatureComparison(temperatures, results, format) {
    ...
    results.forEach((result, index) => {
        ...
        if (result.review_mode) {
            header.innerHTML = `
                <span class="temp-label">Reviewed</span>
                <span class="temp-value">T: ${tempValue} → ${result.reviewer_temperature}</span>
            `;
        } else {
            header.innerHTML = `
        ...
        if (result.error) {
            ...
            if (result.review_mode && result.original_response) {
                ...
                if (result.improvements && result.improvements.length > 0) {
                    ...
                    improvementsSection.innerHTML = `
                        <div class="improvements-header">
                            <span class="improvements-count">${result.improvements_count} improvement${result.improvements_count !== 1 ? 's' : ''}</span>
                        </div>
                        <ul class="improvements-list">
                            ${result.improvements.map(imp => `<li>${escapeHtml(imp)}</li>`).join('')}
                        </ul>
                    `;
                    content.appendChild(improvementsSection);
                }

                // Add collapsible original response
                const originalSection = document.createElement('details');
                originalSection.className = 'original-response';
                originalSection.innerHTML = `
                    <summary>View original response</summary>
                    <div class="original-content">${renderResponse(result.original_response, format, null)}</div>
                `;
        ...
        if (result.tokens) {
            // Handle review mode token display (has agent1/agent2/combined)
            if (result.review_mode && result.tokens.combined) {
                const combined = result.tokens.combined;
                const agent1 = result.tokens.agent1;
                const agent2 = result.tokens.agent2;

                tokenInfo.innerHTML = `
                    <div class="token-stats-review">
                        <div class="token-row">
                            <span class="token-label">Agent 1:</span>
                            <span class="token-item">In: ${agent1.input} Out: ${agent1.output}</span>
                        </div>
                        <div class="token-row">
                            <span class="token-label">Agent 2:</span>
                            <span class="token-item">In: ${agent2.input} Out: ${agent2.output}</span>
                        </div>
                        <div class="token-row token-total">
                            <span class="token-label">Total:</span>
                            <span class="token-item">${combined.total} tokens</span>
                        </div>
                    </div>
