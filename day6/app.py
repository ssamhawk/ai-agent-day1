...
MODEL_CONTEXT_LIMIT = 128000  # gpt-4o-mini context limit
REVIEWER_TEMPERATURE = 0.3  # Lower temperature for more precise reviews

# Reviewer system prompt
REVIEWER_SYSTEM_PROMPT = """You are a critical reviewer agent. Your task is to review and improve the response provided by another AI agent.

Analyze the response and:
1. Check for factual accuracy
2. Identify missing important information
3. Improve clarity and readability
4. Fix any grammar or spelling errors
5. Enhance the overall quality

You MUST respond with a valid JSON object containing:
{
    "improved_response": "The improved version of the response",
    "improvements": ["List of specific improvements made"],
    "improvements_count": <number of improvements>
}

If the original response is already perfect, return it unchanged with improvements_count: 0."""
...
def get_ai_response(user_message, response_format="plain", fields=None, temperature=OPENAI_TEMPERATURE, intelligent_mode=False, max_tokens=OPENAI_MAX_TOKENS):
    """
    ...
    try:
        ...
        # Build messages WITHOUT conversation history for Day 5 (clean comparison)
        messages = [
            {
                "role": "system",
        ...
        if input_tokens + max_tokens > MODEL_CONTEXT_LIMIT:
            available_for_output = MODEL_CONTEXT_LIMIT - input_tokens
            if available_for_output <= 0:
                raise Exception(f"Input too long ({input_tokens} tokens). Maximum context is {MODEL_CONTEXT_LIMIT} tokens.")
...
def get_reviewer_response(original_response, user_message):
    """
    Get a review and improvement of the original response from a reviewer agent.

    Args:
        original_response (str): The original AI response to review
        user_message (str): The original user message for context

    Returns:
        dict: Contains improved response, improvements list, and token information
    """
    try:
        # Build reviewer prompt with context
        review_prompt = f"""Original user question: {user_message}
...
Please review and improve this response."""
        ...
        # Use moderate max_tokens for reviewer
        max_tokens = 2048
        ...
        if input_tokens + max_tokens > MODEL_CONTEXT_LIMIT:
            available_for_output = MODEL_CONTEXT_LIMIT - input_tokens
            if available_for_output <= 0:
                raise Exception(f"Review input too long ({input_tokens} tokens).")
            max_tokens = min(max_tokens, available_for_output)
        ...
        try:
            ...
            improved_response = review_data.get('improved_response', original_response)
            improvements = review_data.get('improvements', [])
            improvements_count = review_data.get('improvements_count', len(improvements))
        except json.JSONDecodeError:
            # If parsing fails, use the raw response
            improved_response = ai_response
            improvements = ["Could not parse structured improvements"]
        ...
        logger.error(f"Error calling reviewer API: {str(e)}", exc_info=True)
        raise Exception(f"Reviewer failed: {str(e)}")
...
def chat():
    ...
    try:
        ...
        temperature = data.get('temperature')  # Get temperature from request
        ...
        max_tokens = data.get('max_tokens')  # Max output tokens
        review_mode = data.get('review_mode', False)  # Review mode - two agents

        # Validate temperature (use default if None)
        ...
        # Validate max_tokens (use default if None)
        if max_tokens is None:
            ...
            try:
                ...
                if max_tokens < 1 or max_tokens > 16384:
                    return jsonify({
                        'error': 'Max tokens must be between 1 and 16384',
                        'success': False
                    }), 400
            except (ValueError, TypeError):
                return jsonify({
        ...
        # Get AI response with specified temperature, format, fields, intelligent mode, and max_tokens
        result = get_ai_response(user_message, response_format, fields, temperature, intelligent_mode, max_tokens)
        ...
        # If review mode is enabled, get reviewer response
        if review_mode:
            try:
                review_result = get_reviewer_response(ai_response, user_message)

                # Calculate total tokens from both agents
                total_tokens = {
                    'agent1': tokens,
                    'agent2': review_result['tokens'],
                    'combined': {
                        'input': tokens['input'] + review_result['tokens']['input'],
                        'output': tokens['output'] + review_result['tokens']['output'],
                        'total': tokens['total'] + review_result['tokens']['total']
                    }
                }

                return jsonify({
                    'response': review_result['improved_response'],  # Final improved response
                    'original_response': ai_response,  # Original response from Agent 1
                    'improvements': review_result['improvements'],
                    'improvements_count': review_result['improvements_count'],
                ...
                logger.error(f"Reviewer failed: {str(e)}")
                # Return original response if reviewer fails
                return jsonify({
                    'response': ai_response,
                    'format': response_format,
                    'parsed': parsed_data,
        ...
        return jsonify({
            ...
            'tokens': tokens,  # Token usage information
            'truncated': truncated,  # Whether response was cut off due to max_tokens
            'temperature': temperature,  # Actual temperature used
            'review_mode': False,
            'success': True
        })
