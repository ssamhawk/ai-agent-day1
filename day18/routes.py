"""
Routes module for Day 12 AI Application
...
from context_compression import get_conversation_state, count_tokens, count_messages_tokens
from config import (
    OPENAI_TEMPERATURE,
    OPENAI_MAX_TOKENS,
    MAX_MESSAGE_LENGTH,
...
from speech_service import transcribe_audio, validate_audio_file
...
def register_routes(app, limiter, client, memory_storage):
    ...
    def chat():
        ...
        try:
            ...
            current_conv_id = memory_storage.active_conversation_id if memory_storage else None
        ...
        try:
            ...
            for conv in conversations:
                try:
                    ...
                    # Compare calendar dates instead of 24-hour periods
                    # This ensures 23:00 yesterday shows in "Yesterday", not "Today"
                    today = now.date()
                    conv_date = last_updated.date()
    ...
    def speech_to_text():
        ...
        try:
            ...
            if not validation['valid']:
                return jsonify({'error': validation['error'], 'success': False}), 400

            logger.info(f"🎤 Received audio file: {validation.get('size', 0) / 1024:.1f}KB, type: {validation.get('extension', 'unknown')}")
