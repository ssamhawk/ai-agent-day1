...
from citation_manager import get_citation_manager
...
Answer:"""
    ...
    def _generate_response_from_chunks(
        ...
        """
        try:
            # Build context with citations using CitationManager
            if not chunks:
                context = "No relevant documents found."
                citation_map = {}
                source_files = []
            else:
                include_rerank = 'rerank_score' in chunks[0]
                context, citation_map, source_files = self.citation_manager.build_context_with_citations(
                    chunks,
                    include_rerank=include_rerank
                )

            # Build chunks_used for response metadata
            chunks_used = []
            ...
            # Create citation-enforcing prompt
            num_sources = len(chunks)
            prompt = self.citation_manager.create_citation_prompt(
                question=question,
                context=context,
                num_sources=num_sources
            )

            # Query LLM with citation-enforcing prompt
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
            ...
            citation_validation = self.citation_manager.validate_citations(
                ...
                num_sources=num_sources,
                strict=False  # Allow partial citations
            )

            # Format sources section
            sources_section = self.citation_manager.format_sources_section(citation_map)
